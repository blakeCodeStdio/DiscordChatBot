...
const cityMapping = {
    ...
    "新竹": "Hsinchu", "宜蘭": "Yilan", "台中": "Taichung", "苗栗": "Miaoli", 
    "彰化": "Changhua", "南投": "Nantou", "雲林": "Yunlin", "嘉義": "Chiayi", 
    "台南": "Tainan", "高雄": "Kaohsiung", "屏東": "Pingtung", "台東": "Taitung", 
    "花蓮": "Hualien", "澎湖": "Penghu", "金門": "Kinmen", "連江": "Lienchiang",
};
...
client.on('messageCreate', async (message) => {
    ...
    const userId = message.author.id; // 獲取用戶 ID
    const userMessage = message.content.trim();
    console.log('收到訊息:', userMessage);

    if (!userMessage) return message.reply('請輸入訊息，我在這裡等你聊天！');

    // **處理天氣查詢**
    if (userMessage.startsWith('!天氣')) {
        let city = userMessage.split(' ')[1];
        ...
        try {
            ...
            const reply = `📍 **${currentWeather.name}** 的天氣：
...
${forecastMsg}`;
    ...
    if (!userConversations.has(userId)) {
        userConversations.set(userId, [systemMessage]); // 初始化對話歷史
    }
    ...
    if (conversationHistory.length > MAX_HISTORY_LENGTH) {
        conversationHistory.splice(1, conversationHistory.length - MAX_HISTORY_LENGTH);
    }
    ...
    try {
        ...
        return message.reply('發生錯誤，請稍後再試！');
    }
});
